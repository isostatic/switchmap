...
sub PutCachedCommunityFirst($) {
  ...
  if ($cachedCommunity ne '') {
    # if it's not in the list already, append it to the front
    if (grep (/^$cachedCommunity$/, @SnmpCommunities) == 0) {
      unshift @ReorderedCommunities, $cachedCommunity;
    } else { # else it is already in the list, so move it to the front
...
sub initialize () {
  ...
  # Return if this is an SNMPv3 site
  return if int($ThisSite::SNMP_version) != 2;
