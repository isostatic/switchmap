...
#
  ...
  if (int($snmp_version) == 2) {
    ...
    $status = SwitchUtils::GetSnmpv3VlanTable($localSession,          # SNMP session
                                              'dot1dBasePortIfIndex', # table name
                                              $Constants::INTERFACE,  # table type
                                              \%BifNbrToIfNbr,        # returned table contents
                                              $VlanNbr);              # VLAN number
...
sub GetOneVlanBridgeTableWithLocalSession ($$$$$) {
  ...
  my $snmp_version = $ThisSite::SNMP_version;
  my $snmp_community = $ThisSite::Community;

  my $localSession;
  my $Error;

  # SNMP communities are per-VLAN, so we need an SNMP session for each VLAN
  $logger->debug("opening a per-VLAN SNMP session to $Switch->{FullName} for VLAN $VlanNbr...");

  if (int($snmp_version) == 2) {
    # Get MAC addresses using community string indexing
    my $VlanCommunity = GetVlanCommunity($Switch, $VlanNbr);
    ($localSession, $Error) = Net::SNMP->session(
                                                 -version    => 'snmpv2c',
                                                 -timeout    => 5,
