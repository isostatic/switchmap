...
my $TotalActivePorts =
  ...
  my $TotalShortTermInactivePorts =
  my $TotalLongTermInactivePorts =
...
#
# Scan the Switches array and collect statistics into %SwitchDetails
#
sub CollectStats ($) {
  ...
  foreach my $Switch (@$SwitchesRef) {
    ...
    my $active = 0;
    my $shortterminactive = 0;
    my $longterminactive = 0;
    my $disabled = 0;
    my $ancillary = 0;
...
#      $logger->debug("port name = $PortName");
    ...
    $Details->{shortterminactive} = $shortterminactive;
    $Details->{longterminactive}  = $longterminactive;
    $Details->{disabled}          = $disabled;
    $Details->{ancillary}         = $ancillary;
    $Details->{total}             = $active + $shortterminactive + $longterminactive + $disabled;
  }
  $logger->debug("TotalActivePorts            = $TotalActivePorts");
  $logger->debug("TotalLongTermInactivePorts  = $TotalLongTermInactivePorts");
  $logger->debug("TotalShortTermInactivePorts = $TotalShortTermInactivePorts");
...
sub WritePortStatesTable () {
  ...
  my $TotalPorts = $TotalActivePorts + $TotalShortTermInactivePorts + $TotalLongTermInactivePorts + $TotalDisabledPorts;
...
<table class="Stats">
  ...
  <tr><td>short-term inactive ports</td><td>$TotalInactiveVirtualPorts</td><td>$TotalShortTermInactivePorts</td></tr>
  <tr><td>long-term inactive ports</td><td>&nbsp;</td><td>$TotalLongTermInactivePorts</td></tr>
...
<table class="Stats">
  ...
  <tr class="tblHead">
    <th>Switch</th>
    <th>active</th>
    <th>inactive<br>&lt; $ThisSite::UnusedAfter days</th>
    <th>inactive<br>&gt; $ThisSite::UnusedAfter days</th>
    <th>disabled</th>
    <th>total</th>
  </tr>
DTOTALS
  ...
  my $lastSite = '';
  my $GrandActive = 0;
  my $GrandShortTermInactive = 0;
  my $GrandLongTermInactive = 0;
  my $GrandDisabled = 0;
  my $GrandAncillary = 0;
  my $GrandTotal = 0;
  my $SiteActive = 0;
  my $SiteShortTermInactive = 0;
  my $SiteLongTermInactive = 0;
  my $SiteDisabled = 0;
  my $SiteAncillary = 0;
  my $SiteTotal = 0;
  foreach my $SwitchDetail (@SwitchDetails) {
    ...
    if ($ThisSite::DnsDomain eq '.ucar.edu') { # if we're at NCAR
      ...
      if ($site ne $lastSite) {
        print SWITCHSTATSFILE <<DDD;
<tr class=\"sitetotals\">
  <td><b>$lastSite totals</b></td>
  <td>$SiteActive</td>
  <td>$SiteShortTermInactive</td>
  <td>$SiteLongTermInactive</td>
  <td>$SiteDisabled</td>
  <td>$SiteTotal</td>
</tr>\n
DDD
        $GrandActive            += $SiteActive;
        $GrandShortTermInactive += $SiteShortTermInactive;
        $GrandLongTermInactive  += $SiteLongTermInactive;
        $GrandDisabled          += $SiteDisabled;
        $GrandAncillary         += $SiteAncillary;
        $GrandTotal             += $SiteTotal;
        $SiteActive = $SiteShortTermInactive = $SiteLongTermInactive = $SiteDisabled = $SiteAncillary = $SiteTotal = 0;
        $lastSite = $site;
      }
    }
    ...
    my $stinactive = $SwitchDetail->{shortterminactive};  print SWITCHSTATSFILE "<td>$stinactive</td>";  $SiteShortTermInactive += $stinactive;
    my $ltinactive = $SwitchDetail->{longterminactive};   print SWITCHSTATSFILE "<td>$ltinactive</td>";  $SiteLongTermInactive  += $ltinactive;
...
GGG
  ...
  print SWITCHSTATSFILE <<HHH;
<tr class=\"grandtotals\">
  <td><b>Grand totals</b></td>
  <td>$GrandActive</td>
  <td>$GrandShortTermInactive</td>
  <td>$GrandLongTermInactive</td>
  <td>$GrandDisabled</td>
  <td>$GrandTotal</td>
</tr>\n
HHH
