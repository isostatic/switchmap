...
use GetVlansOnTrunkPorts;
use GetPortNameToVlanTable;
use Port;
...
#
# Given a speed code that came from the ifSpeed MIB, return a string
# that's easy for a human to understand and is as short as possible,
# so it fits nicely into narrow HTML table cells.
#
sub SpeedMap ($) {
  my $SpeedCode = shift;
...
sub GetPortSpeeds ($$$$$$) {
  ...
  my $MAX_IFSPEED = 4294967295;
  my $hasHighSpeedInterfaces = 0;
  foreach my $ifNbr (keys %ifSpeed) {
    my $ifName = $$IfToIfNameRef{$ifNbr};
    if ($ifSpeed{$ifNbr} == $MAX_IFSPEED) {
      $hasHighSpeedInterfaces = 1;
      last;
    }
  }
  $logger->debug("hasHighSpeedInterfaces = $hasHighSpeedInterfaces");
  # If the switch has one or more high speed interfaces, we need to read the
  # table that contains the higher speed values.
  my %ifHighSpeed;
  if ($hasHighSpeedInterfaces) {
    $logger->info("getting ifHighSpeeds (operational speeds)...");
    $status = SwitchUtils::GetSnmpTable($Session,
                                        'ifHighSpeed',
                                        $Constants::INTERFACE,
                                        \%ifHighSpeed);
    if ($status != $Constants::SUCCESS) { # if we couldn't reach it or it's real slow
      $logger->warn("Couldn't get the ifHighSpeed table from $Switch->{Name}, skipping");
      return $Constants::FAILURE;
    }
  }
  foreach my $ifNbr (keys %ifSpeed) {
    my $ifName = $$IfToIfNameRef{$ifNbr};
    my $speed = $ifSpeed{$ifNbr};
    if ($speed == $MAX_IFSPEED) {
      $IfSpeedsRef->{$ifName} = HighSpeedMap($ifHighSpeed{$ifNbr});
    } else {
      $IfSpeedsRef->{$ifName} = SpeedMap($speed);
    }
  }
...
sub GetPortNameToAuxiliaryVlanTable ($$$$$) {
  ...
  # 2900 MIB, then the switch may not support auxiliary VLANs at all.
  #
...
sub SetExplicitTrunkStatuses($) {
  ...
  if (exists $ThisSite::LocalSwitchTrunkPorts{$SwitchName}) {
    $logger->debug("there is a LocalSwitchTrunkPorts hash");
  ...
  } elsif (exists $ThisSite::LocalSwitchRelaxedTrunkPorts{$SwitchName}) {
    $logger->debug("there is a LocalSwitchRelaxedTrunkPorts hash");
    my $RelaxedPorts = $ThisSite::LocalSwitchRelaxedTrunkPorts{$SwitchName};
    foreach my $PortName (@$RelaxedPorts) {
      my $Port = $Switch->{Ports}{$PortName};
      if (defined $Port) {
        $logger->debug("setting $SwitchName port $PortName as a non-trunk port");
        $Port->{IsTrunking} = 0;
      } else {
        $logger->warn("!!! In ThisSite.pm, in \%LocalSwitchRelaxedTrunkPorts, port $PortName doesn't exist in switch $SwitchName, skipping !!!");
      }
    }
  }
  $logger->debug("returning");
...
#      $logger->debug("\$jnxExVlanPortAccessMode{$port} = $portAccessMode = $textAccessMode");
...
# Override the trunk status of certain explicit ports, regardless of
# what the switch reported via SNMP.
  ...
  if ($Port->{State} ne 'Active') {
    if ($Port->{IdleSince} == -1) {         # if no .idlesince value exists
      ...
      if ($SecondsDead > ($ThisSite::UnusedAfter * $Constants::SecondsPerDay)) {
        $Unused = $Constants::TRUE;
        $Switch->{NbrUnusedPorts}++ if not $Port->{IsVirtual};
      }
    }
  }
  $Port->{Unused}       = $Unused;
...
sub IsVirtualPort ($$) {
  ...
  if ($PortName =~ /^EFXS /) {
    ...
    if (($PortType == 22) or    # proprietary serial
        ...
        ($PortType == 161) or   # Bridge-Aggregation
        ($PortType == 231) or   # Controlled
        ($PortType == 232)) {   # Uncontrolled
...
sub GetInterfaceTypes($$$$) {
  ...
  if (!$status) {
    ...
    foreach my $ifNbr (keys %ifTypes) {
      ...
      # The getIanaIfType function works fine, but I don't have a use
      # for it, so it's commented out for now.
      # my $ifTypeString = $IanaMibConstants::getIanaIfType($ifType);
...
# SwitchUtils::DbgPrintHash('IfToIfName', \%IfToIfName);
# SwitchUtils::DbgPrintHash('IfNameToIf', \%IfNameToIf);
...
#      %lldpRemPortId,
  ...
  $status = SwitchUtils::GetSnmpTable($Session,
                                      ...
                                      \%cieIfDot1dBaseMappingPort);
  if ($status) {
#    SwitchUtils::DbgPrintHash('cieIfDot1dBaseMappingPort', \%cieIfDot1dBaseMappingPort);
    $status = SwitchUtils::GetSnmpTable($Session,
                                        'lldpRemSysName',
                                        $Constants::TABLE_ROW,
                                        \%lldpRemSysName);
    $logger->debug('got lldpRemSysName');
#    SwitchUtils::DbgPrintHash('lldpRemSysName', \%lldpRemSysName);
    ...
    $logger->debug('mapped lldpRemSysName');
#    SwitchUtils::DbgPrintHash('lldpRemSysName', \%lldpRemSysName);
    ...
    $status = SwitchUtils::GetSnmpTable($Session,
                                        ...
                                        $Constants::TABLE_ROW,
                                        \%lldpRemPortDesc);
    $logger->debug('got lldpRemPortDesc');
#    SwitchUtils::DbgPrintHash('lldpRemPortDesc', \%lldpRemPortDesc);
    ...
    $logger->debug('mapped lldpRemPortDesc');
#    SwitchUtils::DbgPrintHash('lldpRemPortDesc', \%lldpRemPortDesc);
    ...
    %lldpRemManAddr = map { # see LLDP-MIB: we exact both the key and value out of the oid/index
        ...
        ( $+{lldpRemLocalPortNum} => $+{lldpRemManAddr} )
      } keys %{$lldpRemManAddrTable};
...
#    SwitchUtils::DbgPrintHash('lldpRemManAddr', \%lldpRemManAddr);
  ...
  #
  my %portNameToVlan;
  GetPortNameToVlanTable::GetPortNameToVlanTable($Switch, $Session, \%IfToIfName, \%portIfIndex, \%portNameToVlan);
  # SwitchUtils::DbgPrintHash('portNameToVlan', \%portNameToVlan);
  ...
  GetVlansOnTrunkPorts::GetVlansOnTrunkPorts($Switch, $Session);
  ...
  foreach my $IfNbr (keys %IfToIfName) {
    ...
    if ($Port->{State} eq 'Active') {
      ...
      $Port->{Speed} = (exists $AdminSpeeds{$PortName}) ? $AdminSpeeds{$PortName} : $IfSpeeds{$PortName};
    }
  }
