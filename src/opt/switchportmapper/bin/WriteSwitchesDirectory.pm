...
sub SwitchInformationTable ($) {
  ...
  # write the ip address into the info header (super useful)
  my $IPaddr = $Switch->GetIPaddr;
  $IPaddr =~ s/\n/<br>\n/g;
  if ($IPaddr) {
    $RetVal .= "<tr><td class=\"tblHead\"><strong>IP&nbsp;Address</strong></td><td>$IPaddr</td></tr>\n";
  }
  
  # If there's a config file for the switch in the nightly backups directory, add a link to the file
  my $ConfigFileName = $Switch->GetName . '-confg';
  if (grep (/^$ConfigFileName/, @ConfigFileNames) == 1) {
    $logger->debug("making a link to $ConfigFileName");
    $RetVal .= "<tr><td class=\"tblHead\"><strong>Config</strong></td><td><a href=\"../configs/$ConfigFileName\">$ConfigFileName</a></td></tr>\n";
  } else {
    $logger->debug("config directory exists, but doesn't contain $ConfigFileName, so I can't make a link to a config file for $ConfigFileName");
  }
...
#
# Loop through the ports in a switch, creating an HTML row for each
# port.  Put each row into a section on the page.  Sections have names
# like 'Module 1', 'Virtual Ports', etc.
#
sub BuildSections ($$$) {
  my $Switch         = shift;
  ...
  %Sections = ();   # clear out any results from processing a previous switch
  foreach my $PortName (Portically::PortSort keys %{$Switch->{Ports}}) {
    ...
    my $SectionName = GetSectionNameForPort($Switch, $PortName);
    if (!exists $Sections{$SectionName}) {
      $Sections{$SectionName} = [ () ];  # make a new, empty array with "()", and save a reference to it
    }
...
sub printModuleSections($) {
  ...
  foreach my $SectionName (Portically::PortSort keys %Sections) {
    if ($SectionName =~ /Module (\d+)/) {
      my $ModNbr = $1;
      print HTMLFILE ModuleList::WriteModuleInformationTable $Switch->{ModuleList}, $SwitchName, $ModNbr;
...
sub WriteSwitchHtmlData ($$$) {
  ...
  print HTMLFILE SwitchInformationTable($Switch);
...
sub WriteSwitchesFiles ($) {
  ...
  if (opendir(DIR,$Constants::ConfDirectory)) {
    $logger->debug("opened $Constants::ConfDirectory");
    @ConfigFileNames = readdir(DIR);
    closedir(DIR);      
    $logger->debug("read in $#ConfigFileNames config file names from $Constants::ConfDirectory");
  } else {
    $logger->debug("couldn't read config directory $Constants::ConfDirectory");
  }
