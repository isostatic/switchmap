  ...
  if ($Port->{IsTrunking}) {
    ...
    if ($NbrMacs == 0) {
      ...
      foreach my $PortMac (keys %{$Port->{Mac}}) {
        ...
        my $oc = (exists $$OuiCodeMapRef {$first6 }) ? $$OuiCodeMapRef {$first6 } : ''; # NIC Manufacturer
        ...
        push @MacIps, join $KludgeDelimeter, $PortMac, $oc, $ia, $hn;
      }
      foreach (sort @MacIps) {
        ($Mac, $Nic, $Ip, $Dns) = split $KludgeDelimeter, $_;
#        $Nic =~ /\&amp;/\&/;
...
# Write a CSV file compatible with RFC 4180.
sub WriteCsvFile ($$$) {
  my $Switch         = shift;
  my $MacIpAddrRef   = shift;
  my $MacHostNameRef = shift;
  ...
  if ($ThisSite::WriteHeaderInCsvFiles) {
    print CSVFILE "SwitchName,PortName,Vlan,State,DaysInactive,Speed,Duplex,PortLabel,WhatViaCdp,Mac,Nic,IP,DNS\n";
  }
